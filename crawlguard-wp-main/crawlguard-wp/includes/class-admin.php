...
class CrawlGuard_Admin {
    ...
    public function init_settings() {
        ...
        // API Base URL (Cloudflare/custom domain)
        add_settings_field(
            'api_base_url',
            'API Base URL',
    ...
    public function api_base_url_callback() {
        $options = get_option('crawlguard_options');
        $value = isset($options['api_base_url']) ? $options['api_base_url'] : 'https://paypercrawl.tech/api';
        echo '<input type="text" name="crawlguard_options[api_base_url]" value="' . esc_attr($value) . '" class="regular-text" placeholder="https://paypercrawl.tech/api" />';
        echo '<p class="description">Base like https://paypercrawl.tech/api or your Cloudflare custom domain. The plugin appends /v1 automatically.</p>';
    }
